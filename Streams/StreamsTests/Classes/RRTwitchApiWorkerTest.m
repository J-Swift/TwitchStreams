...
{
  RRTwitchApiFileSource *source = [RRTwitchApiFileSource new];
  RRTwitchApiWorker *worker = [[RRTwitchApiWorker alloc] initWithApiSource:source];
  ...
  [worker getChannel:@"dendi" onCompletion:^(RRChannel *channel, NSError *error) {
    ...
    XCTAssert([channel.name isEqualToString:name], @"parse failed");
    XCTAssert([channel.imagePath isEqualToString:imagePath], @"parse failed");
    XCTAssert([channel.lastUpdateTime isEqualToDate:date], @"parse failed");
  }];
